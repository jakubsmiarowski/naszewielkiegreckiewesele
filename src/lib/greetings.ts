...
function looksLikeFemaleName(value: string) {
	...
	return Boolean(
		normalized?.endsWith("a") && !MALE_NAMES_ENDING_WITH_A.has(normalized),
	);
}
...
function joinWithAnd(items: string[], locale: GreetingLocale) {
	const connector = locale === "en" ? "and" : "i";
...
export function buildGreeting(
	...
	locale: GreetingLocale = "pl",
) {
	if (!guests.length) return locale === "en" ? "Hi!" : "Cześć!";

	if (locale === "en") {
		const names = guests
			.map((guest) => guest.fullName.split(" ")[0] ?? guest.fullName)
			.filter((name) => Boolean(name));
		if (!names.length) return "Hi!";
		return `Hi ${joinWithAnd(names, locale)}!`;
	}
	...
	const joined = joinWithAnd(names, locale);
	return `Cześć ${joined}!`;
}
